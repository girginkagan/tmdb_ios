...
final class SearchTableViewCell: UITableViewCell {
    ...
    func updateCell(data: SearchResponseModelElement?){
        ivItem.kf.setImage(with: URL(string: baseUrlForImages + ((data?.mediaType ?? MediaType.movie) == MediaType.person ? (data?.profilePath ?? "") : (data?.posterPath ?? ""))), placeholder: UIImage(named: (data?.mediaType ?? MediaType.movie) == MediaType.person ? "ic_cast_placeholder" : "ic_movie_placeholder"))
